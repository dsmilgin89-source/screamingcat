...
use crate::{
    CustomExtractionConfig, CustomSearchConfig, ExtractionMode, ExtractionTarget, SearchMode,
    SearchTarget,
};
...
static SEL_STYLESHEET: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"link[rel="stylesheet"]"#).unwrap());
...
static SEL_VIEWPORT: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"meta[name="viewport"]"#).unwrap());
static SEL_CHARSET: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse("meta[charset]").unwrap());
static SEL_HTTP_EQUIV_CT: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"meta[http-equiv="Content-Type"]"#).unwrap());
static SEL_MICRODATA: LazyLock<Selector> =
...
static SEL_META_ROBOTS: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"meta[name="robots"]"#).unwrap());
static SEL_META_KEYWORDS: LazyLock<Selector> =
...
static SEL_REL_NEXT: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"link[rel="next"]"#).unwrap());
static SEL_REL_PREV: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"link[rel="prev"]"#).unwrap());
static SEL_FORM_ACTION: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse("form[action]").unwrap());
// OG/Twitter meta selectors
static SEL_OG_TITLE_PROP: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"meta[property="og:title"]"#).unwrap());
static SEL_OG_TITLE_NAME: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"meta[name="og:title"]"#).unwrap());
static SEL_OG_DESC_PROP: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"meta[property="og:description"]"#).unwrap());
static SEL_OG_DESC_NAME: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"meta[name="og:description"]"#).unwrap());
static SEL_OG_IMAGE_PROP: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"meta[property="og:image"]"#).unwrap());
static SEL_OG_IMAGE_NAME: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"meta[name="og:image"]"#).unwrap());
static SEL_TW_CARD_PROP: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"meta[property="twitter:card"]"#).unwrap());
static SEL_TW_CARD_NAME: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"meta[name="twitter:card"]"#).unwrap());
static SEL_TW_TITLE_PROP: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse(r#"meta[property="twitter:title"]"#).unwrap());
static SEL_TW_TITLE_NAME: LazyLock<Selector> =
...
static SEL_SOURCE_SRC: LazyLock<Selector> =
    LazyLock::new(|| Selector::parse("source[src]").unwrap());
...
impl HtmlParser {
    pub fn parse(
        ...
        let indexable =
            !robots_meta.contains("noindex") && (canonical.is_empty() || canonical == page_url);
        ...
        let (mixed_content_count, insecure_form_count) =
            Self::extract_security_info(&document, page_url);
    ...
    fn extract_images(doc: &Html, base_url: &Option<Url>) -> (Vec<ImageInfo>, u32, u32) {
        ...
        for element in doc.select(&SEL_IMG) {
            ...
            images.push(ImageInfo { src, alt, has_alt });
        }
    ...
    fn collect_structured_data_items(
            ...
            serde_json::Value::Object(map) => {
                if let Some(t) = map.get("@type") {
                    ...
                    for type_name in &type_names {
                        ...
                        if type_name == "Review"
                            && !map.contains_key("reviewBody")
                            && !map.contains_key("reviewRating")
                        {
                            errors.push(
                                "Missing required property: reviewBody or reviewRating".to_string(),
                            );
                        }
    ...
    fn extract_root_domain(domain: &str) -> String {
        crate::crawler::extract_root_domain(domain)
    }
    ...
    fn run_custom_search(
        ...
        let text_content = if config
            .rules
            .iter()
            .any(|r| r.search_in == SearchTarget::Text)
        {
            doc.select(&SEL_BODY)
                .next()
        ...
        for rule in &config.rules {
            ...
            let search_target = match rule.search_in {
                SearchTarget::Text => &text_content,
                SearchTarget::Html => html,
            };

            let (found, count) = match rule.mode {
                SearchMode::Regex => {
    ...
    fn run_custom_extraction(
        ...
        for rule in &config.rules {
            ...
            let (value, count) = match rule.mode {
                ExtractionMode::CssSelector => {
                    if let Ok(sel) = Selector::parse(&rule.selector) {
                        ...
                        let value = elements
                            .first()
                            .map(|el| match rule.target {
                                ExtractionTarget::InnerHtml => el.inner_html(),
                                ExtractionTarget::Attribute => el
                                    .value()
                                    .attr(&rule.attribute)
                                    .unwrap_or_default()
                                    .to_string(),
                ...
                ExtractionMode::Regex => {
                    if let Ok(re) = regex::RegexBuilder::new(&rule.selector)
                        .size_limit(100_000)
                        .build()
                    {
                ...
                ExtractionMode::Xpath => {
                    // XPath not supported by scraper crate
                    (String::new(), 0)
                }
            };
